...
gdm <- genomic_data_model(window_sizes= c(10, 25, 50, 500, 5000), half_nWindows= c(5, 5, 15, 10, 10)*2 )


file.rdata.model <- "../models/H3k27me3.S6.V2.train.rdata";

selectGPUdevice(0);

if(!file.exists(file.rdata.model))
{
  model <- create_train_model( "/", file.bws.plus, file.bws.minus, file.rdata.negative, file.rdata.positive, "/", file.bw.histone, file.peak.histone, ratio = 0.1, samples=600000, strategy=6, exclude=c("chr22", "chrY", "chrM", "chrUn")  )
  model <- build_train_model( model )
  model <- svm_train_model(model, gdm, file.rdata.model, ncores=15);
  save(model, file=file.rdata.model);
}

red <- svm_predict_chr_parallel( "K562.H3k27me3.S6.V2", file.bws.plus, file.bws.minus, file.rdata.model, chr="chr22", bigwig_compare=file.bw.histone, ncores=5, linear_scale=F, gpu.idx=1  )
