...
file.bw.histone <- file.k562.H3k9me3.bw;
file.peak.histone <- file.k562.H3k9me3.peak;
file.rdata.model <- "../models/H3k9me3.train.S5.rdata";
...
{
  model <- create_train_model( "/", file.bws.plus, file.bws.minus, file.rdata.negative, file.rdata.positive, "/", file.bw.histone, file.peak.histone, 
                                ratio = 0.1, samples=600000, strategy=5, include.bed=df.bed, exclude=c("chr22", "chrY", "chrM", "chrUn")  )
  model <- build_train_model( model )
  model <- svm_train_model(model, gdm, file.rdata.model, ncores=15);
  save(model, file=file.rdata.model);
}

red <- svm_predict_chr_parallel( "K562.H3k9me3.S5", file.bws.plus, file.bws.minus, file.rdata.model, chr="chr22", bigwig_compare=file.bw.histone, ncores=5, linear_scale=F, gpu.idx=1  )
