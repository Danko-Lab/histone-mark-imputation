...
file.model.rdata <- "../models/H3K27ac.S3.V2.train.rdata";
if(!file.exists(file.model.rdata))
{
   model <- create_train_model( "/", file.bws.plus, file.bws.minus, file.rdata.negative, file.rdata.positive, 
        ...
        ratio = 0.1, samples=600000, strategy=3, exclude=c("chr22", "chrY", "chrM"))
...
pred <- svm_predict_chr_parallel( "H3K27ac.S3.V2", file.bws.plus[1], file.bws.minus[1], file.model.rdata, chr="chr22", bigwig_compare=file.all.histone[1], ncores=2, linear_scale=F, gpu.idx=0  )
