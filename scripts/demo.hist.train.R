...
file.bws.plus <- c(file.bw.G1[1], file.bw.G2[1]);
file.bws.minus <-c(file.bw.G1[2], file.bw.G2[2]);

file.bw.histone <- file.H3K27ac.bw;
file.peak.histone <- file.H3K27ac.peak;

model <- create_train_model( path.proseq, file.bws.plus, file.bws.minus, file.rdata.negative, file.rdata.positive, path.histone, file.bw.histone, file.peak.histone, ratio = 0.1, samples=100000 )
model <- build_train_model( model )
model <- svm_train_model(model, gdm, "model.rdata", ncores=15);
